...
from src.types import TangentOrder, WeightFormat

RNG = np.random.default_rng(11)


def _gauss(ts: float, t: np.ndarray, cov_diag: float | np.ndarray) -> GaussianStep:
...
def test_translation_crps_decreases_with_better_predictive():
    ...
    g = translation_crps(step_good, gt, rng=np.random.default_rng(0))
    b = translation_crps(step_bad, gt, rng=np.random.default_rng(0))
    assert g < b
...
def test_rotation_crps_zero_when_concentrated_at_truth():
    ...
    s = rotation_crps(step, gt_q, n_samples=64, rng=np.random.default_rng(0))
    assert s < 1e-3
...
def test_gaussian_log_score_improves_with_centered_truth():
    ...
    s_c = gaussian_log_score(step_centered, gt, np.array([0.0, 0.0, 0.0, 1.0]))
    s_o = gaussian_log_score(step_off, gt, np.array([0.0, 0.0, 0.0, 1.0]))
    assert s_c < s_o
...
def test_ensemble_diagnostics_uniform_weights():
    ...
    particles[:, 6] = 1.0
    weights = np.ones(n) / n
    steps = [EnsembleStep(timestamp=t, particles=particles, weights=weights) for t in (0.0, 1.0)]
...
def test_ensemble_unique_count():
    ...
    diag = ensemble_diagnostics([step], WeightFormat.LINEAR, normalized=True, tol=1e-3)
...
def test_calibration_matches_nominal_when_data_is_drawn_from_predictive():
    ...
    for _ in range(n):
        ...
        mu_q = rot_to_quat_xyzw(
            np.linalg.qr(rng.normal(size=(3, 3)))[0]
        )
