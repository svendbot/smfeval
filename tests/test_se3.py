...
def test_reorder_tangent_involution():
    ...
    back = reorder_tangent(swapped, TangentOrder.ROT_TRANS, TangentOrder.TRANS_ROT)
...
def test_reorder_covariance_block_swap():
    ...
    cov = (cov + cov.T) / 2.0
    swapped = reorder_covariance(cov, TangentOrder.TRANS_ROT, TangentOrder.ROT_TRANS)
    # bottom-right of source should equal top-left of destination
    assert np.allclose(swapped[:3, :3], cov[3:, 3:])
    assert np.allclose(swapped[3:, 3:], cov[:3, :3])
    back = reorder_covariance(swapped, TangentOrder.ROT_TRANS, TangentOrder.TRANS_ROT)
    assert np.allclose(back, cov)


@given(
    st.lists(st.floats(min_value=-2, max_value=2, allow_nan=False), min_size=6, max_size=6)
)
