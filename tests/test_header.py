...
def test_missing_gaussian_field():
    ...
    with pytest.raises(FormatError, match="missing field for gaussian_se3: TANGENT_ORDER"):
        _parse(text)
...
def test_stops_at_first_data_line():
    text = COMMON.format(rep="deterministic", gauge="fixed") + "1.234567 0 0 0 0 0 0 1\n"
    f = io.StringIO(text)
    parse_header(f)
