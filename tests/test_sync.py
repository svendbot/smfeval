...
def test_match_basic():
  ...
  r = match_timestamps(est, ref, t_max_diff=0.001)
  assert r.n_matched == 4
  assert r.n_dropped == 0
  assert list(r.est_indices) == [0, 1, 2, 3]
  assert list(r.ref_indices) == [0, 1, 2, 3]


def test_match_drops_above_tolerance():
  est = np.array([0.0, 1.0, 2.0])
  ref = np.array([0.001, 1.5, 2.001])
  r = match_timestamps(est, ref, t_max_diff=0.01)
...
def test_match_offset_applied():
  ...
  r = match_timestamps(est, ref, t_max_diff=0.001, t_offset=0.5)
  assert r.n_matched == 2


def test_match_empty_when_no_overlap():
  est = np.array([0.0, 1.0])
  ref = np.array([10.0, 11.0])
  r = match_timestamps(est, ref, t_max_diff=0.01)
...
def test_match_iterates_over_shorter():
  ...
  ref = np.array([0.0, 0.1, 0.2])  # 3 samples — the shorter
  r = match_timestamps(est, ref, t_max_diff=0.01)
  assert r.n_matched == 3
  assert sorted(r.ref_indices.tolist()) == [0, 1, 2]


def test_match_gap_quantiles():
  est = np.array([0.0, 1.0, 2.0])
  ref = np.array([0.001, 1.002, 2.005])
  r = match_timestamps(est, ref, t_max_diff=0.01)
...
def test_sync_risk_zero_when_dt_zero():
  ...
  est_ts = ref_ts.copy()
  est_steps = [
    _gauss_step(t, p, 1.0) for t, p in zip(est_ts, ref_pos, strict=False)
  ]
  r = sync_risk(
    est_steps,
    ref_ts,
