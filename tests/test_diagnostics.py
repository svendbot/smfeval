...
def _slice(verdict: str, anees: float, *, dof: int = 3, lo: float = 2.9,
           hi: float = 3.1, nees_median: float | None = None) -> dict:
  return {
    "anees": anees, "dof": dof, "n": 100, "lo": lo, "hi": hi,
    "verdict": verdict, "calibration_median": anees / 2.0,
...
def test_regime_tail_routes_to_student_t():
  ...
  rep = Report(calibration_split={"absolute": {
...
def test_local_overconfidence_from_windowed():
  rep = Report(calibration_split={
    "absolute": {"joint": _slice("consistent", 6.0, dof=6),
                 "translation": _slice("consistent", 3.0),
                 "rotation": _slice("consistent", 3.0)},
    "windowed": [
      {"window_s": 0.1, "anees": 1900.0, "dof": 3, "n": 2800, "lo": 2.9,
       "hi": 3.1, "verdict": "optimistic", "calibration_median": 390.0,
       "sharpness_median": -14.0, "sigma_rel_m": 0.009},
    ],
  })
  ...
  rep = Report(calibration_split={
    ...
    "windowed": [
      {"window_s": 0.1, "anees": 1900.0, "dof": 3, "n": 2800, "lo": 2.9,
       "hi": 3.1, "verdict": "optimistic", "calibration_median": 390.0,
       "sharpness_median": -14.0, "sigma_rel_m": 0.009},
      {"window_s": 10.0, "anees": 13000.0, "dof": 3, "n": 2700, "lo": 2.9,
       "hi": 3.1, "verdict": "optimistic", "calibration_median": 3500.0,
       "sharpness_median": -25.0, "sigma_rel_m": 0.009},
    ],
  })
  modes = _modes(rep)
  ...
  rep = Report(calibration_split={
    ...
    "windowed": [
      {"window_s": 0.1, "anees": 1900.0, "dof": 3, "n": 2800, "lo": 2.9,
       "hi": 3.1, "verdict": "optimistic", "calibration_median": 390.0,
       "sharpness_median": -14.0, "sigma_rel_m": 0.009},
      {"window_s": 10.0, "anees": 2100.0, "dof": 3, "n": 2700, "lo": 2.9,
       "hi": 3.1, "verdict": "optimistic", "calibration_median": 420.0,
       "sharpness_median": -14.0, "sigma_rel_m": 0.009},
    ],
  })
...
def test_gaussian_tangent_invalid_critical():
  rep = Report(gaussian_validity={
    "n_total": 1000, "n_exceeding_soft": 200, "n_exceeding_hard": 200,
  })
  d = next(d for d in diagnose(rep)
           if d.mode == FailureMode.GAUSSIAN_TANGENT_INVALID)
  assert d.severity == Severity.CRITICAL


def test_sync_risk_fires_only_for_nearest():
  rep = Report(sync={
    "n_matched": 1000, "risk_excess_count": 500, "risk_threshold": 0.3,
    "mode": "nearest",
  })
  assert FailureMode.SYNC_RISK in _modes(rep)
  rep_interp = Report(sync={
    "n_matched": 1000, "risk_excess_count": 500, "risk_threshold": 0.3,
    "mode": "interpolate_gt",
  })
...
def _bv(bias_fraction: float, axis: str) -> dict:
  return {"window_s": 0.1, "n_pairs": 100, "mse": 1.0,
          "bias_fraction": bias_fraction, "bias": [0.1, 0.0, 0.5],
          "std": [0.1, 0.1, 0.1], "dominant_axis": axis}
