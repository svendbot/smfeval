r"""Prequential score-series summary statistics with a stationary block bootstrap.

Politis & Romano (1994) on the mean, calibrated by an automatic
block-length estimator (Politis & White, 2004).
...
def _autocov(x: np.ndarray, max_lag: int) -> np.ndarray:
  r"""Biased sample autocovariance for :math:`k = 0, \ldots, M`.

  :math:`\hat R(k) = N^{-1}\sum_{t} x_t x_{t+k}`. ``x`` is assumed
  already centred.
  """
  n = x.size
...
def _flat_top_window(z: np.ndarray) -> np.ndarray:
  r"""Flat-top lag window (Politis & White, 2004, eq. 4).

  :math:`\lambda(z) = 1` for :math:`|z| \le 1/2`, :math:`2(1 - |z|)` for
  :math:`1/2 < |z| \le 1`, else 0.
  """
  az = np.abs(z)
...
def politis_white_block_length(values: np.ndarray | list[float]) -> float:
  r"""Politis & White (2004) optimal mean block length for the stationary bootstrap.
...
def summarize(
  ...
  *,
  ...
  r"""TUM-style summary statistics with a stationary-bootstrap CI on the mean.
