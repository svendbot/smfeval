...
def parse_header(f: TextIO) -> SquareHeader:
  raw: dict[str, str] = {}
  while True:
    pos = f.tell()
    line = f.readline()
...
def write_header(f: TextIO, h: SquareHeader) -> None:
...
def _build(raw: dict[str, str]) -> SquareHeader:
  ...
  if rep is Representation.GAUSSIAN_SE3:
    ...
    if "PARTICLE_COUNT_HINT" in raw:
      try:
        ...
        raise FormatError(
          f"PARTICLE_COUNT_HINT not an integer: {raw['PARTICLE_COUNT_HINT']!r}"
        ) from e
