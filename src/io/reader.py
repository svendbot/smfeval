...
from src.types import FormatError, Header, Representation


def iter_steps(f: TextIO, header: Header) -> Iterator[Step]:
...
def _iter_gaussian(f: TextIO) -> Iterator[GaussianStep]:
    for line in _iter_with_truncation(_data_lines(f), 29):
        parts = line.split()
        if len(parts) != 29:
            raise FormatError(f"gaussian_se3 row has {len(parts)} fields, expected 29")
        try:
...
def _iter_deterministic(f: TextIO) -> Iterator[DeterministicStep]:
    for line in _iter_with_truncation(_data_lines(f), 8):
        parts = line.split()
        if len(parts) != 8:
            raise FormatError(f"deterministic row has {len(parts)} fields, expected 8")
        try:
            vals = [float(p) for p in parts]
        except ValueError as e:
            raise FormatError(f"non-numeric value in row: {line!r}") from e
...
def _iter_ensemble(f: TextIO, header: Header) -> Iterator[EnsembleStep]:
