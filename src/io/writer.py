...
from src.types import Header, Representation
...
def write_step(f: TextIO, step: Step, header: Header) -> None:
...
def write_steps(f: TextIO, steps: Iterable[Step], header: Header) -> None:
    for step in steps:
...
def _write_ensemble(f: TextIO, step: EnsembleStep, header: Header) -> None:
