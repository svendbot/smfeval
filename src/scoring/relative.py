...
from src.scoring.crps import _gaussian_crps
from src.scoring.logscore import AneesResult, _score_components, anees_consistency
from src.scoring.summary import ScoreSummary, summarize
...
def relative_translation_crps(
  ...
  *,
  ...
  for w in windows_s:
    ...
    sigma_rel = np.sqrt((sigma**2).sum(axis=1))  # ‖σ‖ per pair
