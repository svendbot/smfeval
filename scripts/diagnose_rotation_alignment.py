...
Given a smfeval estimate file and a TUM ground-truth, prints the geodesic
...
Usage:
  python scripts/diagnose_rotation_alignment.py <est.smfeval> <gt.tum>
"""
import sys
...
from src.align import fit_alignment, propagate_step
from src.io import iter_steps, parse_header
from src.io.reader import _iter_deterministic
from src.scoring import rotation_crps
from src.sync import match_timestamps
...
def main(est_path: Path, gt_path: Path) -> None:
    ...
    raw = [
        ...
        for s, q in zip(matched, mq)
    ]
    print(f"{'raw':12s}: geodesic median {np.median(raw):.4f} rad  "
    ...
    for mode in ("gravity_yaw", "se3"):
        ...
        ang = [
            ...
            for s, q in zip(aligned, mq)
        ]
        rng = np.random.default_rng(0)
        crps = [rotation_crps(s, q, eh.tangent_order, 32, rng)
                for s, q in zip(aligned, mq)]
        print(f"{mode:12s}: geodesic median {np.median(ang):.4f} rad  "
